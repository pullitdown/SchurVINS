...
namespace schur_vins
{
    ...
    bool ZUPTUpdater::try_update(ZUPTUpdater::Ptr last_state_ptr,Eigen::MatrixXd &conv, double now_timestamp)
    {
        ...
        double time0 = last_state_ptr->ts + last_prop_time_offset;
        double time1 = now_timestamp;

        // Select bounding inertial measurements
        std::vector<IMUDATA> imu_recent = select_imu_readings(imu_data, time0, time1);

        // Move forward in time
        VLOG(40)<<imu_recent.size();

        // Check that we have at least one measurement to propagate with
        if (imu_recent.empty() || imu_recent.size() < 2)
        {
            VLOG(0)<<"imu_recent empty or date items num less than 2";
            return false;
        }
...
*/
        ...
        for (size_t i = 0; i < imu_recent.size() - 1; i++)
        {
            ...
            Eigen::Vector3d a_hat = imu_recent.at(i).am - last_state_ptr->ba;

            // Measurement residual (true value is zero)
            res.block(6 * i + 0, 0, 3, 1) = -(imu_recent.at(i).wm -  last_state_ptr->bg);
            if (!integrated_accel_constraint)
            {
                res.block(6 * i + 3, 0, 3, 1) = -(a_hat +  last_state_ptr->quat.conjugate() * _gravity);

                VLOG(40)<<a_hat<<" quat"<<last_state_ptr->quat.w()<<" "<<last_state_ptr->quat.x()<<" "<<last_state_ptr->quat.y()<<" "<<last_state_ptr->quat.z() \
                <<"\n_gravity"<<last_state_ptr->quat.conjugate() * _gravity<<"\na:"<<imu_recent.at(i).am;
            }
            else
            {
                res.block(6 * i + 3, 0, 3, 1) = -(last_state_ptr->vel + _gravity * dt + last_state_ptr->quat * a_hat * dt);
            }

            // Measurement Jacobian
            Eigen::Matrix3d R_GtoI_jacob = last_state_ptr->quat_fej.conjugate().toRotationMatrix();
        ...
        VLOG(40)<<"R"<<R;
        // if(!integrated_accel_constraint)
        Eigen::MatrixXd P_marg = Eigen::MatrixXd::Zero(h_size, h_size);
        
        P_marg.block(0,0,3,3).noalias()=conv.block(0,0,3,3);
        P_marg.block(3,0,3,3).noalias()=conv.block(12,0,3,3);
        P_marg.block(6,0,3,3).noalias()=conv.block(9,0,3,3);

        P_marg.block(0,3,3,3).noalias()=conv.block(0,12,3,3);
        P_marg.block(3,3,3,3).noalias()=conv.block(12,12,3,3);
        P_marg.block(6,3,3,3).noalias()=conv.block(9,12,3,3);

        P_marg.block(0,6,3,3).noalias()=conv.block(0,9,3,3);
        P_marg.block(3,6,3,3).noalias()=conv.block(12,9,3,3);
        P_marg.block(6,6,3,3).noalias()=conv.block(9,9,3,3);
        ...
        if (chi2 > _options.chi2_multipler * chi2_check || last_state_ptr->vel.norm() > _zupt_max_velocity)
        {
        VLOG(0)<<"[ZUPT]: rejected zero velocity |v_IinG| = "<<last_state_ptr->vel<<" (chi2 "<< chi2 <<" < "<<_options.chi2_multipler * chi2_check <<")\n";
        ...
        Eigen::VectorXd diags = P_marg.diagonal();
        ...
        VLOG(40)<<"[ZUPT]: accepted zero velocity |v_IinG| = "<<last_state_ptr->vel<<" (chi2 "<<chi2 <<" < "<<_options.chi2_multipler * chi2_check <<")\n";

        Eigen::MatrixXd M_a = Eigen::MatrixXd::Zero(h_size, res.rows());
        // For each active variable find its M = P*H^T
        M_a=P_marg*H.transpose();//hsize×msize

        

        VLOG(40)<<"M_a=P_marg*H.transpose()\nM_a::"<<M_a;
    ...
    // Invert our S (should we use a more stable method here??)
        ...
        Eigen::MatrixXd K = M_a * Sinv.selfadjointView<Eigen::Upper>();//hsize*msize
    //Eigen::MatrixXd K = M_a * S.inverse();

    // Update Covariance


        P_marg -= K * M_a.transpose();
        
        VLOG(40)<<"P_marg -= K * M_a.transpose()\nK："<<K<<"\n\nres"<<res<<"\n\nP_marg"<<P_marg;

        conv.block(0,0,3,3)=P_marg.block(0,0,3,3);
        conv.block(12,0,3,3)=P_marg.block(3,0,3,3);
        conv.block(9,0,3,3)=P_marg.block(6,0,3,3);

        conv.block(0,12,3,3) =P_marg.block(0,3,3,3);
        conv.block(12,12,3,3)=P_marg.block(3,3,3,3);
        conv.block(9,12,3,3)=P_marg.block(6,3,3,3);

        conv.block(0,9,3,3)=P_marg.block(0,6,3,3);
        conv.block(12,9,3,3)=P_marg.block(3,6,3,3);
        conv.block(9,9,3,3)=P_marg.block(6,6,3,3);

        Eigen::VectorXd diags_ = P_marg.diagonal();
        bool found_neg_ = false;
        for(int i=0; i<diags_.rows(); i++) {
        if(diags(i) < 0.0) {
            found_neg_ = true;
            }
        }
        ...
        last_state_ptr->quat=updateQuaternionWithDelta(last_state_ptr->quat.conjugate(),dx.block(0,0,3,1)).conjugate();
        last_state_ptr->bg+=dx.block(3,0,3,1);
        last_state_ptr->ba+=dx.block(6,0,3,1);

        VLOG(20)<<"update dx"<<dx<<std::endl;

        last_state_ptr->ts = now_timestamp;
        CHECK(dx.block(6,0,3,1).norm()<0.5)<<"update dx"<<dx<<"\nres"<<res;

        VLOG(40)<<"last_state_ptr update\n";
    ...
    {
        zuptupdater_.reset(new ZUPTUpdater(Eigen::Vector3d(0, 0, -9.79007),0.1,1));
    }
    ...
    bool SchurVINS::try_zerovelocity_update(double timestamp)
    {
        if(zuptupdater_!=nullptr)
        {
            return zuptupdater_->try_update(curr_state,cov,timestamp);
        }
        return false;
    }
    ...
    void SchurVINS::Prediction(double _dt, const Eigen::Vector3d &_acc, const Eigen::Vector3d &_gyr)
    {
        ...
        bool z_pos_vel_constrain = 0;
        if (z_pos_vel_constrain)
        {
            Eigen::Vector3d previous_pos = curr_state->pos;
            Eigen::Vector3d previous_vel = curr_state->vel;
            Eigen::Vector3d previous_acc = curr_state->acc; // 保存前一时刻的加速度

            // state prediction, 4th order Runge-Kutta
            PredictionState(acc, gyr, _dt);

            // 计算位置变化
            double z_pos_change_norm = curr_state->pos.z() - previous_pos.z();
            double is_positive = z_pos_change_norm > 0 ? 1 : -1;
            double contrain_z_change = 0.01;
            // 判断前后两帧的加速度方向是否相反
            if (previous_acc.dot(acc) < 0)
            {
                // 如果加速度方向相反，将速度设为零，并限制位置变化
                curr_state->vel.setZero();
                if (z_pos_change_norm > contrain_z_change || z_pos_change_norm < -contrain_z_change)
                {

                    curr_state->pos.z() = previous_pos.z() + is_positive * 0.02;
                    curr_state->vel.setZero();
                }
            }
            else
            {
                ...
                if (z_pos_change_norm > contrain_z_change || z_pos_change_norm < -contrain_z_change)
                {
                    curr_state->pos.z() = previous_pos.z() + is_positive * contrain_z_change;
                    curr_state->vel.z() = is_positive * (contrain_z_change / _dt);
                }
            }
        }
        else
        ...
        cov.block(0, 0, 15, 15) = Phi * cov.block(0, 0, 15, 15) * Phi.transpose() + Phi * G * imu_noise * G.transpose() * Phi.transpose() * _dt;
    ...
    void SchurVINS::Solve3()
    {
        ...
        for (svo::LocalPointMap::iterator pit = local_pts.begin(); pit != local_pts.end(); ++pit)
        {
            ...
            if (curr_pt->register_id_ != curr_state_id)
            { // init point jacobian
                curr_pt->gv.setZero();
                curr_pt->V.setZero();
                curr_pt->W.setZero();
            ...
            if (curr_pt->CheckStatus() == false || curr_pt->CheckLocalStatus() == false || curr_pt->CheckLocalStatus(prev_frame_id0, prev_frame_id1, max_frame_idx) == false)
            {
                 ...
                 ++fit)
            {
                ...
                if (curr_frame_idx < min_frame_idx || curr_frame_idx > max_frame_idx) // pass observations on schurvins local sliding window
                    continue;
                ...
                ++num_obs;
                dpc_dpos.leftCols(3).noalias() = R_i_c.transpose() * Utility::SkewMatrix(Pi) * ft_state->quat.toRotationMatrix().transpose();
    ...
    void SchurVINS::RegisterPoints(const svo::FrameBundle::Ptr &frame_bundle)
    {
        ...
        for (const svo::FramePtr &frame : frame_bundle->frames_)
        {
            ...
            for (size_t i = 0; i < frame->numFeatures(); ++i)
            {
                ...
                {
                    const svo::LocalFeaturePtr ft = std::make_shared<svo::LocalFeature>(state_ptr, point, frame, frame->f_vec_.col(i), camera_id);
    ...
    void SchurVINS::InitState(double _ts, const Eigen::Vector3d &_acc, const Eigen::Vector3d &_gyr)
    {
        ...
        LOG(INFO) << "InitState acc: " << _acc[0] << ", " << _acc[1] << ", " << _acc[2] << ", gyr: " << _gyr[0] << ", " << _gyr[1]
                  << ", " << _gyr[2];
    ...
    void SchurVINS::Forward(const svo::FrameBundle::Ptr frame_bundle)
    {
        ...
        double img_ts = frame_bundle->getMinTimestampSeconds() - delay_imu_cam;
        for (size_t i = 0; i < frame_bundle->imu_datas_.size(); i++)
        {
            ...
            { // Interpolation img time imu data
...

        {
            ...
            LOG(INFO) << "schurvins forward: " << std::fixed << std::setprecision(6) << curr_state->ts << ", quat: " << quat.w()
    ...
    {
        AugmentState(frame_bundle);   // add state into statemap
        RegisterPoints(frame_bundle); // add point into state in statemap ,add in loacl map,add a observation to a point ptr

        int idx = 0;
        std::for_each(states_map.begin(), states_map.end(),
                      [&](svo::StateMap::value_type &it)
                      { it.second->index = idx++; }); // reindex the states map
        ...
        for (const svo::FramePtr &frame : frame_bundle->frames_)
        {
            ...
            for (size_t i = 0; i < frame->numFeatures(); ++i)
            {
                ...
                if (curr_pt && local_pts.find(curr_pt->id()) != local_pts.end())
                {
                    ...
                    const Eigen::Vector2d r = (obs.head<2>() / obs.z() - Pc.head<2>() / Pc.z()) * (focal_length / level_scale);
                    total_error += r.norm();
